...
<html lang="en">
    ...
    <header id="header" class="navbar navbar-default  navbar-fixed-top" role="banner">
          ...
          <div class="container">
                        ...
                        <ul class="nav navbar-nav navbar-right">
                            ...
                            <li id="main-menu">
                                <ul class="nav nav-pills">
                                    ...
                                    <li role="presentation"><a href="{{$menu->link}}" {{($menu->ajaxify)? 'data-ajaxify="true"' : null}}>{{$menu->icon}} {{$menu->name}}</a> </li>
                                    @endforeach
                                </ul>
                            </li>
                            @endif
            ...
            @if(Auth::check())
            <ul class="nav navbar-barra">
             <li class="dropdown account-menu notification-link">
                <a data-toggle="dropdown" data-hover="dropdown" class="dropdown-toggle" title="Tu Cuenta" href="">
                    <img src="{{Auth::user()->present()->getAvatar(50)}}"/>
                </a>
                <ul class="dropdown-menu">
            ...
            <li id="notification-link" class="notification-link">
                    ...
                    <a href="" id="notification-dropdown-trigger" title="Últimas Notifiaciones" class="notification-dropdown-trigger"><i class="icon ion-ios-world"></i>
            ...
            <li class="notification-link">
                    ...
                    <a class="friend-request-trigger" id='friend-request-trigger' title="Solicitudes de Amistad" href="{{URL::route('friend-requests')}}"><i class="icon ion-person-stalker"></i> {{(!empty($countRequest)) ?'<span>'.$countRequest. '</span>' : null}}</a>
            ...
            <li id="message-dropdown-link" class="notification-link">
                    ...
                    <a href="" class="new-messages-trigger" id='new-messages-trigger' title="Bandeja de Mensajes"><i class="icon ion-chatboxes"></i>
            ...
            @foreach(Menu::lists('menu-barra') as $menu)
            <li><a href="{{$menu->link}}" title="{{$menu->title}}" {{($menu->ajaxify)? 'data-ajaxify="true"' : null}}>{{$menu->icon}} {{$menu->name}}</a> </li>
            @endforeach
          </ul>
          @endif
